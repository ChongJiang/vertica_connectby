...
#
# Makefile to build connectby user defined functions
#
# To run under valgrind:
#   make RUN_VALGRIND=1 run
#
# Copyright 2013 Vertica, 2013
############################
...
PACKAGE_LIBNAME=lib/connectby.so
...
# run examples
run: $(PACKAGE_LIBNAME) uninstall install test/test.sql
	$(VSQL) -f test/test.sql | tee testresult.txt
	# $(VSQL) -f test/sampleTraceRows.sql | tee -a testresult.txt
	# $(VSQL) -f test/sampleGraph.sql | tee -a testresult.txt

run.oracle: test/test.oracle.sql
	sqlplus system/oracle@192.168.116.2:1521/orcl < test/test.oracle.sql | tee testresult.oracle.txt
